    ...
    SubShader {
        ...
        void vert( inout appdata_full v)
        {
           ...
           // vv.xyz -= _WorldSpaceCameraPos.xyz;
            ...
            float xd = abs(vv.x - planets[0].x) * planets[0].y;
            xd += abs(vv.x - planets[1].x) * planets[1].y;
            float zd = abs(vv.z - planets[0].z) * planets[0].y;
            zd += abs(vv.z - planets[1].z) * planets[1].y;

            float d = 2-((zd-1) + (xd-1));

            float reald = d * (step(d, 0));

            vv.y -= reald * _Curvature;
